...
// ---- resident memory (MB) ---------------------------------------------------
// Use phys_footprint (TASK_VM_INFO), the metric Activity Monitor's "Memory"
// column reports — the true, dirty+compressed footprint of the process. The
// older MACH_TASK_BASIC_INFO.resident_size is badly inflated on Apple Silicon /
// recent macOS: it counts shared and reclaimable pages and drifts upward over a
// long run even when the real footprint is flat, which reads as a phantom leak.
static double residentMB()
{
    ...
    // Fallback if TASK_VM_INFO is unavailable.
